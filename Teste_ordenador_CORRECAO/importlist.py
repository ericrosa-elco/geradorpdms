...
def pagina_principal():
    ...
    with st.sidebar:
        pagina_selecionada = option_menu(
            "Navegação",
            options=["Início", "Gerador de Descrição", "Cadastro/Edição de Itens", "Mensagens", "Usuários", "Terminar sessão"],
            icons=["house", "tools", "plus-circle", "chat-left-text", "people", "box-arrow-right"],
            menu_icon="cast",
            default_index=0,
            styles={
        ...
        if not insumos:
            ...
            if not insumos_filtrados:
                ...
                for idx, item in enumerate(insumos_filtrados):
                    ...
                    with st.expander(f"🔹 {item['nome_item']} ({item['status'].capitalize()})", expanded=expandir):
                        ...
                        if perfil_logado in ["adm", "adm_master"] and item['status'] == "pendente":
                            ...
                            if col_aprovar.button("✅ Aprovar", key=f"aprovar_{idx}"):
                            ...
                            if col_rejeitar.button("❌ Rejeitar", key=f"rejeitar_{idx}"):
